...
Re-enroll behavior: - If data/enroll/<name> already contains aligned crops, those are loaded,
 embedded again, and INCLUDED in the template. New captures are appended.

Outputs: - data/db/face_db.npz   (name -> embedding vector) - data/db/face_db.json  (metadata)
Optional: - data/enroll/<name>/*.jpg aligned face crops

Controls: - SPACE: capture one sample (if face found) - a: auto-capture toggle (captures periodically) - s: save enrollment (after enough total samples) - r: reset NEW samples (keeps existing crops on disk) - q: quit
"""
...
def load_db(cfg: EnrollConfig) -> Dict[str, np.ndarray]:
    if cfg.out_db_npz.exists():
        try:
            ...
            # Corrupted or empty file -> return empty DB
            return {}
    return {}
