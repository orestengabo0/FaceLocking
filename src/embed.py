...
camera -> Haar detection -> FaceMesh 5pt -> align_face_5pt (112x112) -> ArcFace embedding -> vector visualization (education) 
...
def download_model(model_path: str):
    print(f"[embed] Downloading ArcFace model to {model_path}...")
    try:
        urllib.request.urlretrieve(MODEL_URL, model_path)
        print("[embed] Download complete.")
    except Exception as e:
        print(f"[embed] Failed to download model: {e}")
