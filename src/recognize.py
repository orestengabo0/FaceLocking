...
Haar (multi-face) -> FaceMesh 5pt (per-face ROI) -> align_face_5pt (112x112) -> ArcFace ONNX embedding -> cosine distance to DB -> label each face.
...
Notes: - We run FaceMesh on EACH Haar face ROI (not the full frame). This avoids the
 “FaceMesh points not consistent with Haar box” problem and enables multi-face. - DB is expected from enroll: data/db/face_db.npz (name -> embedding vector) - Distance definition: cosine_distance = 1 - cosine_similarity.
...
self.sess.get_outputs()[0].type)
    ...
    def _ensure_model(self, model_path: str):
        # reuse the embed.py logic if possible, or just duplicate import
        from .embed import download_model
        path = Path(model_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        if not path.exists() or path.stat().st_size == 0:
            download_model(str(path))
